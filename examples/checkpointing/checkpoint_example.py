import torch
import os, sys
from cluster import update_params_from_cmdline, exit_for_resume, save_metrics_params
...
if __name__ == '__main__':
    ...
    while True:
        # do some training
        x = torch.normal(0,1., size=(128, 10))
        y = model(x)
        ...
        # It is best to replace the iteration- with a time-constraint, jobs grow in cost based on runtime
        # by (0.1 * running_bid * n_compute_units) every hour!
        if iteration == 100:
